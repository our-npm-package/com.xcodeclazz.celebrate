...
const { CelebrateError, isCelebrateError } = require("./CelebrateError");
...
internals.partialValidate = (steps, req) =>
  steps.reduce(
    (chain, validate) =>
      chain.then(() =>
        validate(req)
...
exports.celebrate = (_requestRules, joiOpts = {}, opts = {}) => {
  ...
  Object.entries(_requestRules).reduce(
    (memo, [key, value]) => memo.set(key, Joi.compile(value)),
    requestRules
  );
...
// We have to add this to keep our error schema balanced as other follows
// the same patter for sending error
exports.celebrate_custome_errors = (opts = {}) => {
  const finalOpts = { ...internals.DEFAULT_ERRORS_OPTS, ...opts };
  Joi.assert(finalOpts, ERRORSOPTSSCHEMA);
  ...
  return (err, req, res, next) => {
    ...
    return res.status(statusCode).send({
      errors: _.map(validation, (e) => {
        return { message: e.message || err.message, field: e.keys.join(" ") };
      }),
    });
    // ////////////////////////////
  };
